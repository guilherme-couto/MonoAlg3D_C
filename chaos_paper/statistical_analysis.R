...
# ==========================================================
REF_FIBROSIS   <- "Stochastic"
REAL_REF       <- "Diffuse"    # Reference when Stochastic is removed from GAM
MIN_EVENTS_GAM <- 5
...
if (has_angles) {
  ...
  if (ANGLE_COL %in% colnames(data_angles)) {
    colnames(data_angles)[colnames(data_angles) == ANGLE_COL] <- "angle"
  } else if (!"angle" %in% colnames(data_angles)) {
    stop(paste("Error: Angle column", ANGLE_COL, "not found in CSV."))
  }

  data_angles$fibrosis_type <- capitalize(trimws(as.character(data_angles$fibrosis_type)))
  data_angles$fibrosis_type <- factor(data_angles$fibrosis_type, levels = FIB_ORDER)
...
# 1. Fit for all active types (including Stochastic)
if (length(active_types) > 0) {
  df_all <- data %>% filter(fibrosis_type %in% active_types) %>% droplevels()
  tryCatch({
    model_all <- gam(cbind(successes, failures) ~ fibrosis_type + s(density, by=fibrosis_type, k=5), family = quasibinomial(link = "logit"), data = df_all)
  ...
  }, error = function(e) { cat("Error fitting GAM (All):", e$message, "\n") })
}

# 2. Fit for realistic types (Excluding Stochastic)
real_types <- active_types[active_types != REF_FIBROSIS]
if (length(real_types) > 0) {
  df_real <- data %>% filter(fibrosis_type %in% real_types) %>% droplevels()
  if(REAL_REF %in% levels(df_real$fibrosis_type)) df_real$fibrosis_type <- relevel(df_real$fibrosis_type, ref = REAL_REF)
  tryCatch({
    model_real <- gam(cbind(successes, failures) ~ fibrosis_type + s(density, by=fibrosis_type, k=5), family = quasibinomial(link = "logit"), data = df_real)
  ...
  }, error = function(e) { cat("Error fitting GAM (Realistic):", e$message, "\n") })
}

# Append rare types
if (length(rare_types) > 0) {
  rare_data <- expand.grid(fibrosis_type = factor(rare_types, levels = FIB_ORDER), density = seq(min(data$density), max(data$density), length.out = 200), prob = 0, ci_lower = 0, ci_upper = 0)
...
if (has_angles) {
  cat("\n=== RUNNING GAM (ANGLE-SPECIFIC CURVES) ===\n")
  angles_present <- sort(unique(as.numeric(as.character(data_angles$angle))))

  for (ang in angles_present) {
    df_ang <- data_angles %>% filter(angle == as.character(ang))
    ...
    if (length(rar_types) > 0) gam_angle_plot_data_all <- bind_rows(gam_angle_plot_data_all, expand.grid(fibrosis_type=rar_types, density=seq(min(df_ang$density), max(df_ang$density), length.out=200), prob=0, ci_lower=0, ci_upper=0, angle=as.character(ang)))
    ...
    if (length(rar_real) > 0) gam_angle_plot_data_real <- bind_rows(gam_angle_plot_data_real, expand.grid(fibrosis_type=rar_real, density=seq(min(df_ang$density), max(df_ang$density), length.out=200), prob=0, ci_lower=0, ci_upper=0, angle=as.character(ang)))
  }
...
} else { cat("No overall GAM fitted.\n") }
...
} else { cat("No realistic density curves modeled.\n") }
...
# PLOT 1: GAM CURVES
plot_gam_curves(gam_plot_data_all, "Statistical Model", "statistical_model_fit_all.png")
plot_gam_curves(gam_plot_data_real, "Statistical Model of Arrhythmia Risk", "statistical_model_fit_realistic.png")
...
if (!is.null(odds_ratios)) {
  ...
  p_forest <- ggplot(or_df, aes(x = OR, y = fibrosis_type, color = fibrosis_type)) +
    ...
    labs(title = "Scenario III - Relative Arrhythmia Risk", subtitle = "Odds Ratios with 95% Confidence Intervals\nBaseline: Stochastic (OR = 1.0)",
    ...
    theme(axis.text.x = element_text(size=14, color="black", face="bold"), axis.text.y = element_text(size=16, color="black", face="bold"),
          axis.title = element_text(size=16), legend.position = "none", legend.justification = "left", legend.text=element_text(size=14, color=FIB_COLORS[REF_FIBROSIS]),
          panel.grid.minor = element_blank())
  ggsave(file.path(input_dir, "odds_ratio_forest_plot.png"), plot = p_forest, width = 8, height = 5, dpi = 300)
}

# PLOT 3: OVERALL INCIDENCE BAR PLOT - WITH DASHED THRESHOLD
...
p_bar <- ggplot(bar_data, aes(x = fibrosis_type, y = Total_Successes, fill = fibrosis_type)) +
  ...
  labs(title = "Scenario III - Overall Arrhythmia Incidence", x = "Fibrosis Pattern", y = "Total Number of Sustained Reentries") +
  theme_minimal(base_family = "sans", base_size = 16) +
...
if (has_angles) {
  ...
  plot_gam_angles(gam_angle_plot_data_all, "Scenario III - Arrhythmia Risk by Fiber Orientation (All Patterns)", "statistical_model_fit_by_angle_all.png")
  plot_gam_angles(gam_angle_plot_data_real, "Scenario III - Arrhythmia Risk by Fiber Orientation (Realistic)", "statistical_model_fit_by_angle_realistic.png")
  ...
  p_bar_angle <- ggplot(event_counts_angle, aes(x=fibrosis_type, y=Total_Successes, fill=angle)) +
    ...
    labs(title="Scenario III - Arrhythmia Incidence by Pattern and Fiber Orientation", x="Fibrosis Pattern", y="Total Number of Sustained Reentries") +
    theme_minimal(base_family="sans", base_size=16) +
    theme(axis.text.x=element_text(size=14, color="black", face="bold"), axis.text.y=element_text(size=14, color="black", face="bold"),
